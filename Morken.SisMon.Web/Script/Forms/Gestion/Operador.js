...
function ObtenerFiltros() {
    ...
    obj_Filtro.NumeroDocumento = null;
...
function ListarOperador(int_pNumPagina) {
    ...
    AccionDefault(true, "Operador.aspx/ListarOperador", obj_Data, CargarGrilla, null, null, null, 1);
}

function CargarGrilla(obj_pLista) {

    var obj_TablaPrincipal = $("<table id='tblOperador' class='table table-bordered table-hover dt-responsive'></table>");
    var obj_THead = $("<thead></thead>");
    var obj_FilaPrincipal = $("<tr></tr>");
    obj_FilaPrincipal.append($("<th style='width:10%;'>Tipo Documento</th>"));
    obj_FilaPrincipal.append($("<th style='width:10%;'>Documento</th>"));
    obj_FilaPrincipal.append($("<th style='width:20%;'>Nombre</th>"));
    ...
    for (var i = 0; i < obj_pLista.Result.length; i++) {
        ...
        obj_FilaPrincipal = $("<tr value='" + obj_pLista.Result[i].IdOperador + "'></tr>");
        obj_FilaPrincipal.append($("<td style='text-align:center;'>" + obj_pLista.Result[i].Persona.TipoDocumento.Descripcion + "</td>"));
    ...
    var obj_Tabla = $('#tblOperador').dataTable({
        "language": {
    ...
    $('#tblOperador tbody').on('click', 'tr', function () {
...
function obtenerIdRegistro() {
    var str_Id = null;
    var obj_Seleccion = $('#tblOperador').find('.selected');
    if (obj_Seleccion.length > 0)
        str_Id = $(obj_Seleccion[0]).attr('value');
    else
        AlertJQ(1, 'Seleccione un operador.');
    return str_Id;
}
...
function Activar() {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de activar al operador?', ModificarEstado, [str_Id, 'A']);
    }
}
...
function Inactivar(int_pId) {
    var str_Id = obtenerIdRegistro();
    if (str_Id != null) {
        ConfirmJQ('¿Está seguro de inactivar al operador?', ModificarEstado, [str_Id, 'I']);
    }
}

function ModificarEstado(str_pId, str_pEstado) {
    Accion('Operador.aspx/ModificarEstado', {
        "int_pIdOperador": str_pId,
